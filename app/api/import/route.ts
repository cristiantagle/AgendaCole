...
export async function POST(req: Request) {
  ...
  try {
    mapping = mappingStr ? JSON.parse(mappingStr) : null;
  } catch {
    mapping = null;
  }
  ...
  for (const r of rows) {
    ...
    const direccion = norm(
      src['direccion'] || src['Dirección'] || src['DIRECCIÓN'] || src['Dirección Colegio'] || src['ubicacion'] || src['Ubicación'] || src['UBICACION']
    );
